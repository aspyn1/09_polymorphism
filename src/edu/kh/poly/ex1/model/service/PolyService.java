...
public class PolyService {
	...
	public void ex1() { // 다형성 확인 예제
		...
		// 상속 + 다형성이 되어있는 상태로
		// 상속특징 : Car 상속클래스는 모두 getEngine()을 가지고 있다를 정의
		// 다형성(업캐스팅) : 부모타입 참조변수 arr[i]로 자식 객체를 참조할 수 있다. 
		
		}
	
	
	public void ex2() { // 3) 다형성(업캐스팅)을 이용한 매개변수 사용법
	...
	public void ex3() {
		...
		Car c1 = new Tesla("전기모터", "전기", 4, 50000000);
		
		System.out.println(((Tesla)c1).getBatteryCapacity());
		// 주의!! . 연산자가 형변환 연산자보다 우선순위가 높음 -> ()로 한 번 더 처리해줘야함
		
		
		Tesla t1 = (Tesla)c1; // 다운캐스팅 후 얕은복사
...
//		Spark s1 = (Spark)c1;
	...
	public void ex5() {
		// 바인딩 : 실제 실행할 메소드 코드와 호출하는 코드를 연결시키는 것
		...
		// c1.getEngine()호출코드와
		// edu.kh.poly.ex1.mode.vo.Car.getEngine() 메소드 코드를 연결
		// -> 정적 바인딩
		
		System.out.println(c1.toString());
		// String edu.kh.poly.ex1.mode.vo.Car.toString()
		// Car 참조변수 c1을 이용해서
		// Car 객체에있는 오버라이딩된 toString() 메소드를 호출
		...
		// 하지만 실행해보면 자식Spark의 toString이 호출되는 것을 확인할 수 있다.
